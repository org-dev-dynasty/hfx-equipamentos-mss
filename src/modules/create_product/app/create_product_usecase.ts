  ...
  async execute(
    ...
    if (attributes) {
      attributes = attributes.map(attribute => {
        if (models) {
          return { ...attribute, modelId: models.find(model => model.includes(attribute.modelId)) }
        }
        if (categories) {
          return { ...attribute, categoryId: categories.find(category => category.includes(attribute.categoryId)) }
        }
        return attribute
      })
    }
