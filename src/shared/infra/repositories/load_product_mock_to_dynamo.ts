...
import { ProductRepositoryMock } from './product_repository_mock'
import { ProductRepositoryDynamo } from './product_repository_dynamo'

async function setupDynamoTable(): Promise<void> {
  const dynamoTableName = env.DYNAMO_PRODUCTS_TABLE_NAME
...
async function loadMockToRealDynamo() {
  const dynamoRepo = new ProductRepositoryDynamo()
  ...
  //   count += 1
  // }
  const mock = new ProductRepositoryMock()
  const products = await mock.getAllProducts()
  ...
  for(const product of products) {
    // console.log(`Loading product ${product.id} | ${product.name} to dynamoDB...`)
    await dynamoRepo.createProduct(product)
    count += 1
  }

  console.log(`${count} loaded to real DynamoDB`)
